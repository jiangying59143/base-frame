...
import com.yjiang.base.core.common.annotion.Permission;
import com.yjiang.base.core.common.constant.Const;
import com.yjiang.base.core.common.constant.dictmap.UserDict;
import com.yjiang.base.core.common.constant.factory.ConstantFactory;
import com.yjiang.base.core.common.constant.state.ManagerStatus;
...
import com.yjiang.base.core.util.AvatarUtils;
import com.yjiang.base.modular.system.factory.UserFactory;
import com.yjiang.base.modular.system.model.User;
import com.yjiang.base.modular.system.service.IUserService;
import com.yjiang.base.modular.system.transfer.UserDto;
import com.yjiang.base.modular.system.warpper.UserWarpper;
...
public class UserMgrController extends BaseController {
    ...
    public String userInfo(Model model) {
        ...
        model.addAttribute("roleName", ConstantFactory.me().getRoleName(user.getRoleid()));
        model.addAttribute("deptName", ConstantFactory.me().getDeptName(user.getDeptid()));
        model.addAttribute("avatar", AvatarUtils.setAvatarPath(AliyunOSSClientUtil.IMAGE_BUCKET_NAME, user.getAvatar(), request));
        LogObjectHolder.me().set(user);
    ...
    public ResponseData add(@Valid UserDto user, BindingResult result) {
        ...
        // 完善账号信息
        user.setSalt(ShiroKit.getSalt());
        user.setPassword(ShiroKit.md5(user.getPassword(), user.getAccount() + user.getSalt()));
    ...
    @ResponseBody
    public ResponseData edit(@Valid UserDto user, BindingResult result) throws NoPermissionException, IOException {
        if (result.hasErrors()) {
            throw new ServiceException(BizExceptionEnum.REQUEST_NULL);
        }
        ...
        if(!user.getAvatar().equals(oldUser.getAvatar())){
            AliyunOSSClientUtil.upload(AliyunOSSClientUtil.IMAGE_BUCKET_NAME, user.getAvatar(), new File(gunsProperties.getFileUploadPath() + user.getAvatar()));
        }
...
//        user.setSalt(ShiroKit.getRandomSalt(5));
        user.setSalt(ShiroKit.getSalt());
        user.setPassword(ShiroKit.md5(Const.DEFAULT_PWD, user.getAccount() + user.getSalt()));
