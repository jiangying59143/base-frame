...
(function ($, window) {
    ...
    functionality.montage = function (citydata, first) {
        var self = this,
        ...
        $.each(data, function (key, value) {
            name = value.name ; //显示的是简称还是全称
            if (citydata < 1) {

                if (first === key) {

                    $.each(value, function (lkey, lvalue) {
                        name = lvalue.name; //显示的是简称还是全称
                        html += '<a href="javascript:;" class="caller"  data-lat="'+lvalue.lat + '" data-lng="'+lvalue.lng +'" data-parentid="' + lvalue.id + '" data-id="' + lvalue.id + '" data-title="' + name + '" data-first="' + lvalue.first + '">' + name + '</a>';
                    });

                }
                
            } else {
                html += '<li class="caller"  data-lat="'+lvalue.lat + '" data-lng="'+lvalue.lng +'" data-parentid="' + value.id + '" data-id="' + value.id + '" data-title="' + name + '" data-first="' + value.first + '">' +
    ...
    functionality.template = [
        '<div class="city-pavilion hide">',
            '<div class="city-tabs">',
                '<a href="javascript:;" class="tab-a active" data-first="HOT">热门城市</a>',
                '<a href="javascript:;" class="tab-a" data-first="AB">AB</a>',
                '<a href="javascript:;" class="tab-a" data-first="CD">CD</a>',
                '<a href="javascript:;" class="tab-a" data-first="EFG">EFG</a>',
                '<a href="javascript:;" class="tab-a" data-first="HI">HI</a>',
                '<a href="javascript:;" class="tab-a" data-first="JK">JK</a>',
                '<a href="javascript:;" class="tab-a" data-first="LM">LM</a>',
                '<a href="javascript:;" class="tab-a" data-first="NOP">NOP</a>',
                '<a href="javascript:;" class="tab-a" data-first="QR">QR</a>',
                '<a href="javascript:;" class="tab-a" data-first="S">S</a>',
                '<a href="javascript:;" class="tab-a" data-first="TU">TU</a>',
                '<a href="javascript:;" class="tab-a" data-first="VWX">VWX</a>',
                '<a href="javascript:;" class="tab-a" data-first="Y">Y</a>',
                '<a href="javascript:;" class="tab-a" data-first="Z">Z</a>',
            '</div>',
            '<div class="city-cont">',
                '{cont}',
            '</div>',
    ...
    functionality.tabs = function (event) {
        ...
        //显示对应索引的城市列表
        this.$selector.find('dl').addClass('hide').siblings('.city-'+ first).removeClass('hide');
    ...
    functionality.singleAchieve = function (event) {
        ...
        //存储选中的值
        this.values = [];
        this.values.push({ 'lat': lat,'lng': lng,'name': name, 'id': id, 'parentId': parentId });
    ...
    functionality.forbid = function () {
        ...
        $.each(self.$selector.find('.city-cont').find('dl'), function (key, value) {
            ...
            if (!$(value).text()) {
                self.$selector.find('.tab-a[data-first="'+ _first +'"]').addClass('forbid');
            }
        });
    ...
    Cityselect.prototype.groupCity = function () {
        ...
        for ( var i = 0; i < functionality.groupArray.length; i++ ) {
            ...
            //不是热点城市的字母就不做拆分处理
            firstStr = groupArray !== 'HOT' ? functionality.split(groupArray) : '';

            //添加隐藏class
            hide = i > 0 ? ' hide' : '';

            //创建对应的dl
            domtel += '<dl class="city-'+ groupArray + hide +'" data-first="'+ groupArray +'">{dl}</dl>';
            ...
            if (firstStr && firstStr.length > 1) {
                ...
                for (var j = 0; j < firstStr.length; j++) {
                    ...
                    montage = functionality.montage.call(self, 0, firstStr[j]);

                    //不为空就执行下面创建节点
                    if (montage) {

                        list += '<dt>'+ firstStr[j] +'</dt>'+
                                '<dd>'+ montage +'</dd>';
